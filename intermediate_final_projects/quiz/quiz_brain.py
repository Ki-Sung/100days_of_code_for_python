...
# 필요 속성: quetion_number - 퀴즈 문제 순서 / question_list - 질문 리스트 
...
class QuizBrain:                                    # QuizBrain 이라는 클래스 생성 
    # 초기화 메소드 
    def __init__(self, q_list):                     # question_number(기본값이 0이기 때문에 여기서 생략), question_list, 두 속성을 초기화 하는 init 메소드 (입력 값 입력)
        self.question_number = 0                    # 속성 설정 1. 문제 번호를 위한 "question_number"
        self.question_list = q_list                 # 속성 설정 2. quiz 데이터를 뽑기 위한 "question_list"
    ...
    def next_question(self):
        current_question = self.question_list[self.question_number]                     # 현재 퀴즈 질문 - question_list[question_number]
        self.question_number += 1                                                       # 문제 번호 더하기 - 해당 설정을 하지 않으면, Q.0으로 넘버링이 됨 
        input(f"Q.{self.question_number}: {current_question.text} (True/False): ")      # 입력할 정답 호출 